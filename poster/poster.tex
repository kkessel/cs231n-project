...
\item \textbf{Outputs:}
    ...
    \item sensitivity $S_{C,k}^\ell(\mathbf{x})$ of the model's prediction of image $\mathbf{x}$ as class $k$ to concept $C$
    	\[ S_{C,k}^\ell(\mathbf{x}) = \nabla h_k^\ell\left(f_\ell(\mathbf{x})\right) \cdot \mathbf{v}_C^\ell \]
    \item TCAV score aggregating sensitivities for each training image $\mathbf{x}$ in class $k$
...
\begin{figure}%
    ...
    \qquad
    \subfloat{\includegraphics[width=.2\textwidth]{img/cav_viz2}}}%
    \qquad
    \subfloat{\includegraphics[width=.2\textwidth]{img/blue2s}}\
    \caption{\textcolor{cardinal}{(a)} Colorized MNIST training set for classification of hand-written digits \textcolor{cardinal}{(b)} Concept (left) and non-concept (right) training sets to learn CAVs for the concept blue \textcolor{cardinal}{(c)} Data set used to test the relevance of the concept blue in the classification of images belonging to class 2 }%
    \label{fig:example}%
\end{figure}
...
\begin{figure}%
    ...
    \qquad
    \subfloat[Accuracy of model trained on blue-biased data set and tested on color-biased data sets]{{\includegraphics[width=.4\textwidth]{img/data_distr_blue2_test_new2} }}%
%    \caption{2 Figures side by side}%
    \label{fig:example}%
\end{figure}
...
\item \textbf{Output sets:}
	\begin{itemize}
	\footnotesize{
	\item $\mathcal{Y}_{\pm v, \ell} = \{ y \in \mathcal{D}_{\vec y_\ell} \mid \pm y^{T}v \geq 0 \}$. This set corresponds to the positive ($+$) or negative ($-$) halfspace defined by the vector $v$ in the space of the activations of layer $\ell$. 
    }
	\end{itemize}
\end{itemize}
